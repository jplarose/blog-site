...
describe("template-layout helpers", () => {
  ...
  it("stores a positive column span for blocks inside column layouts", () => {
    const layout = createLayout();
  ...
  it("updates a column block's configured column count", () => {
    const layout = insertBlock(
      createLayout(),
      {
        id: "column-c",
        kind: "column",
    ...
    const updatedLayout = updateBlockProps(layout, "column-c", {
      direction: "row",
      columns: 3,
    });

    expect(updatedLayout.blocks["column-c"]?.kind).toBe("column");
    expect(updatedLayout.blocks["column-c"]?.props).toEqual({
      direction: "row",
      columns: 3,
    });
  });
